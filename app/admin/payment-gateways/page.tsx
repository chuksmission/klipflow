...
export default function AdminPaymentGateways() {
  ...
  const gateways = [
    ...
    },
    {
      ...
      name: "Flutterwave",
      icon: "??",
      desc: "Africa + Global payments",
      fields: [
        { key: "flutterwave_public_key", label: "Public Key", secret: false },
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold mb-1">Payment Gateways</h1>
          <p className="text-gray-400 text-sm">Configure and enable payment providers</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
        ...
        >
          {saving ? "Saving..." : saved ? "? Saved!" : "Save Gateways"}
        </button>
      </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs">{gateway.desc}</p>
                  </div>
                </div>
                <button
                  onClick={() => setEnabled({ ...enabled, [gateway.id]: !enabled[gateway.id] })}
                  className={"relative w-12 h-6 rounded-full transition-colors " + (enabled[gateway.id] ? "bg-purple-600" : "bg-white/20")}
                >
