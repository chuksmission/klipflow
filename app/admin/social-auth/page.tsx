...
export default function AdminSocialAuth() {
  ...
  const providers = [
    ...
    },
    {
      ...
      fields: [
        ...
        { key: "apple_team_id", label: "Team ID", secret: false },
        { key: "apple_key_id", label: "Key ID", secret: false },
        { key: "apple_private_key", label: "Private Key", secret: true },
      ],
    },
    {
      id: "phone",
      name: "Phone OTP via Twilio",
      icon: "??",
      desc: "Sign in with phone number via SMS OTP",
      redirect: null,
      fields: [
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (!session) return;
      const res = await fetch("/api/admin/settings?category=social_auth", {
          ...
          {providers.map((provider) => (
            <div key={provider.id} className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => setEnabled({ ...enabled, [provider.id]: !enabled[provider.id] })}
                  className={"relative w-12 h-6 rounded-full transition-colors " + (enabled[provider.id] ? "bg-purple-600" : "bg-white/20")}
                >
