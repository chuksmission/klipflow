...
// Safe JSON parse — never throws on empty body
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (model === "higgsfield-ugc") {
      ...
      // Use the correct Higgsfield SDK endpoint pattern
      ...
      const higgsfieldBody: Record<string, unknown> = {
        prompt,
        aspect_ratio: aspect_ratio === "16:9" ? "landscape" : aspect_ratio === "9:16" ? "portrait" : "square",
        duration: parseInt(duration),
        enhance_prompt: true,
      };

      if (image_url) {
        higgsfieldBody.input_images = [image_url];
      }
      ...
      console.log("Higgsfield response status:", higgsfieldResponse.status);
      console.log("Higgsfield response body:", JSON.stringify(higgsfieldData));

      if (!higgsfieldResponse.ok) {
        if (user_id && tokens_used > 0) await refundTokens(user_id, tokens_used);
        const errMsg = higgsfieldData.error ?? higgsfieldData.message ?? higgsfieldData.detail ?? `Higgsfield API error (${higgsfieldResponse.status})`;
        return NextResponse.json({ error: errMsg, refunded: true }, { status: higgsfieldResponse.status });
      }

      // Higgsfield returns generation_id or id
      const taskId = higgsfieldData.generation_id ?? higgsfieldData.id ?? higgsfieldData.request_id;
      if (!taskId) {
        if (user_id && tokens_used > 0) await refundTokens(user_id, tokens_used);
        return NextResponse.json({ error: "Higgsfield did not return a task ID. Check API credentials.", refunded: true }, { status: 500 });
      }

      return NextResponse.json({
        success: true,
        task_id: taskId,
        status: higgsfieldData.status ?? "processing",
        provider: "higgsfield",
      });
    }
